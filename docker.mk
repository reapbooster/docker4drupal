...
.PHONY: install-npm
install-npm: 
	docker exec -u=0 -ti $(shell docker ps --filter name='$(PROJECT_NAME)_$(or $(filter-out $@,$(MAKECMDGOALS)), 'php')' --format "{{ .ID }}") apk add --update npm
	docker exec -u=0 -ti $(shell docker ps --filter name='$(PROJECT_NAME)_$(or $(filter-out $@,$(MAKECMDGOALS)), 'php')' --format "{{ .ID }}") npm install --global webpack-cli
